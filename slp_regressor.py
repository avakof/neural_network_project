...
class SimpleSLPRegressor(BaseSLPEstimator):
    """
    Simple feed-forward neural-network regressor.
    ...
    def _forward_propagation(self, X: NDArray[np.floating]) -> Tuple[
        ...
        hidden_zs, activations, z2, y_pred : tuple
            Hidden pre-activations, layer activations, output logits, and predictions
        """
    ...
    def _backward_propagation(
        ...
        delta = y_pred - y
        gradients_w[-1] = (a1[-1].T @ delta) * scale
        gradients_b[-1] = np.sum(delta, axis=0) * scale

        for layer_idx in range(len(self.hidden_layer_sizes) - 1, -1, -1):
            delta = (delta @ self.weights_[layer_idx + 1].T) * activation_derivative(
                z1[layer_idx]
            )
            gradients_w[layer_idx] = (a1[layer_idx].T @ delta) * scale
            gradients_b[layer_idx] = np.sum(delta, axis=0) * scale
